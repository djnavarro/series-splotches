...
#   - adapted from curled_16
#   - minor tinkering to settings
...
is_within_heart <- function(x, y, d) {
  ...
  for(th in seq(0, 2*pi, .025)) {
    xv <- heart_x(th)
    yv <- heart_y(th)
    for(d_test in seq(d/10, d, length.out = 10)) { # 10 bands
      hit <- sqrt((xv*d_test-x)^2 + (yv*d_test-y)^2) < d/50  # d/50 makes them thin
      inside <- inside | hit
    }
  }
  return(inside)
...
create_image_parameters <- function(seed) {
  ...
  list(
    ...
    layer_shape_contrast = sample_uniform(min = .1, max = .2),
