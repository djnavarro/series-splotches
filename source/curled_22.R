...
#   - adapted from curled_21
#   - novelty version that always uses bi flag colours
...
#    dplyr::select(-source) |>
#    unlist()
  palette_base <- c("#d60270", "#9b4f96", "#0038A8")
  palette_base <- sample(palette_base)
  palette_base <- as.vector(t(replicate(2, palette_base)))
#  palette_base <- c("white", palette_base)
...
is_along_grid <- function(x, y, d, seed) {
  ...
  xv <- data$x + .005 * curl$x
  yv <- data$y + .005 * curl$y
  zv <- data$z + .005 * curl$z
  zv <- normalise(zv, min = d/4, max = d/3) * 1.5

  # find points inside a fixed radius of curled grid points
  inside <- rep(FALSE, length(x))
  for(i in seq_along(xv)) {
    hit <- sqrt((xv[i] - x)^2 + (yv[i] - y)^2) < zv[i]
    inside <- inside | hit
  }
  return(inside)
}
...
create_layered_image <- function(data, seed, shape_size, shape_contrast) {
  data |>
    dplyr::mutate(
      ...
      value = ifelse(shape, value, value - .5),
